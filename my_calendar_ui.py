...
def renderOneLineList(draw, items, count):
    for j in range(0, count):
        ...
        if showBorder:
            ...
            draw.line((viewport['width'] - CONFIG['listItem']['height'], itemPosition + (CONFIG['listItem']['height'] - 14) / 2, viewport['width'], itemPosition + (CONFIG['listItem']['height'] - 14) / 2), outline = FILL_BLACK)
            draw.line((viewport['width'] - CONFIG['listItem']['height'] / 2, itemPosition, viewport['width'], itemPosition + CONFIG['listItem']['height']), outline = FILL_BLACK)
