...
import sys
import os
resourcesdir = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'resources')
...
try:
    ...
    font35 = ImageFont.truetype(os.path.join(resourcesdir, 'Font.ttc'), 35)
    ...
    Himage = Image.open(os.path.join(resourcesdir, '4in2.bmp'))
    epd.display(epd.getbuffer(Himage))
    time.sleep(2)

    logging.info("4.read bmp file on window")
    Himage2 = Image.new('1', (epd.height, epd.width), 255)  # 255: clear the frame
    bmp = Image.open(os.path.join(resourcesdir, '100x100.bmp'))
    ...
    #display 4Gra bmp
    Himage = Image.open(os.path.join(resourcesdir, '4in2_Scale_1.bmp'))
